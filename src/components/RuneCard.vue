...
const runeBgImgStyle = computed(() => {
	return {
		backgroundImage: `url(${runesFlatMap.get(rune.value.primary_rune_ids[0])?.icon.toLowerCase()})`,
...
<template>
	<div class="flex flex-col">
		...
		<div class="flex flex-col apply">
			<n-button secondary type="primary" size="small" @click="() => emit('apply', rune)">应用</n-button>
		</div>
	</div>
</template>
